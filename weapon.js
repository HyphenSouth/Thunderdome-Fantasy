...
var defaultWeaponOdds = [
    ["knife",30],["gun",20],["lance",25],["bow",20],["wand",18],["cross",20],["rake",20],["guitar",0],["surfboard",0],
    ["katana", 25], ["shotgun", 25], ["sniper",20], ["dnagun",0],
...
DNA_ATTR =[
    ["big guy",3],["retard",5],["blurry eyes",10],["chad",4],["aids",2],["brap",5]
]

class DNAGun extends Weapon {
    constructor() {
        super("dnagun");
        log_message('dna gun created')
    }
    ...
    effect(state, data={}){
        ...
        switch(state){
            case "dealDmg":            
                ...
                let dna_type = roll(DNA_ATTR);
                log_message('dna attack')
                log_message(dna_type)
                if(oP.has_attr("DNAMan")){
                    oP.health+=20;
                }
                else if(oP.has_attr(dna_type) || oP.get_status_effect('dna')!=""){
                     oP.inflict_status_effect(new Poison(2, 4, this.player))
                }
                else{
                    this.player.statusMessage = "gives " + oP.name + " " + dna_type + " DNA";
                    oP.inflict_status_effect(new DNAEff(this.player,dna_type))
                }
                this.use();
                break;
            default:
    ...
    stat_html(){
        let html = super.stat_html()+
        "<span>Alters target DNA</span><br>"
        return html;
    }
}

class DNAEff extends StatusEffect{
    constructor(owner, dna_type){
        super("dna", 10, 20000);
        this.icon="🧬"
        this.owner=owner;
    ...
    afflict(player){
		this.player=player;
		if(this.dna_type!='blurry eyes'){
            if(!this.player.has_attr(this.dna_type))
                this.player.attributes.push(create_attr(this.dna_type, this.player))
        }
        else{
            this.sightBonus = -20;
        }
    ...
    stat_html(){
        let html= super.stat_html() +
        "<span class='desc'>"+
            "<span>DNA altered</span><br>"+
            "<span>DNA:</span>"+this.dna_type+"<br>"+
        "</span>"        
        return html;
    }
}